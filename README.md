...
Underneath those five, one purpose: **ai-flow exists to produce the best code that can be maintained and scaled over time, at a reasonable cost and at an acceptable speed.** Quality is the objective — architecture, simplicity, readability, code a stranger can change a year from now without fear. Cost and speed are the budgets that keep it honest: unbounded, the pursuit of quality becomes the pursuit of perfection, and what is perfect but unaffordable never ships. They never lower the bar — when one is reached, the work gets smaller rather than worse.

One thing it deliberately is **not**: coordination as an end in itself. Reporting, allocation and ceremony that exists so that people can be seen to be in step spend both budgets and improve no code, while coordination that makes the work better — a second pair of eyes on a change — is method and earns its place like anything else. And one thing it is **not yet**: independent of the tools it runs on. ai-flow is Claude-Code-native today, and no rule in it may depend on a particular model, harness or number of people; where one still does, that is debt being paid down rather than a property being defended.
