...
| Workflow arguments | `global/workflows/verify-review.js` (the header comment and each `a.*` read), `global/skills/verify/SKILL.md` (step 7), `docs/architecture/verify.md`, `docs/context/context.md` (its read block, where `changedFiles` is named as what a task touches in Verify) — this card's artifacts table names them too, and a card that omitted itself here would be the miscount it exists to prevent |
