...
5. **Follow the protocol**:
   ...
   - Load what the `steering:` map names for each affected domain — **the map's value, whatever path it names**; `.ai-flow/steering/<domain>.md` is the convention, not the only place an entry may point. Nano block first, full body if the domain is touched.
