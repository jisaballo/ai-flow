...
5. **Conform** (after plan approval): **Resolve context first** — `protocols/context.md` > `## Reading` owns the set, the cuts and the read-once rule; restate none of it here. Then generate test stubs (GWT format) from the `understand.md` Verifiable Criteria (which arrive in EARS), keyed on each criterion's `observed:` — the protocol's `## Conformance Tests` owns which values emit a row, what a stub's body must assert, and how a criterion that emits none is recorded. Then **freeze the contracts**: write the baseline manifest to `artifacts/T-XXX/conformance-baseline/manifest.md` — a manifest, not file copies, whose fields are the protocol's `### The frozen row` and are named there and not here, so a field added to the row moves one file. Execute becomes "make the tests pass" — goal-directed. Then **advance the sheet to EXECUTE**: Conform's close is where that position becomes true, and nothing later in the chain reaches back to record it — a command reads the position it runs on and never writes it — which is why the audit would otherwise wait on a position no one produces.
