...
4. **Criterion audit (YOU do this — it needs the full task context, which only this session holds)**: for each criterion, cite evidence (`file:line`, test name, or observable behavior) and mark ✅ / ⚠️ / ❌. Re-run every Verify command from each `plan.md` step (catches cross-step regressions a later step may have introduced). Then two more checks from the protocol:
   - **Contract check**: diff current conformance specs against `artifacts/T-XXX/conformance-baseline/manifest.md` — every frozen row and every recorded absence keeps the fields the protocol's contract-check step names, and that step names them; divergence without an `## Implementation Decisions` entry → ❌.
...
5. **Decide whether to skip the multi-agent review.** Skip it for: Quick-path tasks, Auto-level tasks — the level as read from the task's sheet at step 2, never inferred here from how the work looks (rely on test validation only), and reached only where the operator asked for this phase on an Auto task, since the path itself skips it — or pure style/i18n/config changes with no logic. Otherwise continue.
...
7. **Invoke the verify-review workflow** (deterministic 5-auditor + adversarial refutation). **Copy the script into the scratchpad first** — `cp ~/.claude/workflows/verify-review.js <scratchpad>/verify-review.js` — and pass that path. The Workflow tool accepts only a path it returned or one you can already read, which `~/.claude/` is not: passing the installed path directly is refused, and the refusal names no remedy. Copy per run, never once and reused, or an edit to the installed workflow silently stops reaching the review. Call the **Workflow** tool with:
   ...
   - `args`: `{ taskId, area, understandPath, planPath, contextPaths, changedFiles, diffText, testCommand, contractChecklist, coverageChecklist, securityChecklist, architectureChecklist, structureChecklist }`
   Resolve `area` from the affected unit using `area_kind` in `.ai-flow/project.yml`. Resolve **`contextPaths`** — a list of `{ key, path }` entries, in resolution order — by the rule in `~/.claude/ai-flow/protocols/context.md` > `## Reading`, **which owns the set, the order and the read-once rule, and restates none of them here**: a citation that summarises what it cites is a second copy wearing a pointer's clothes, and this phase would then resolve by the copy while the others resolve by the rule. Two facts belong to this call rather than to that document, and are stated here for that reason. **A path that does not read is named in the run's own output and dropped from the list**, never dropped in silence: a path nobody looked up cannot be reported missing. **Test each resolved path before the Workflow call** — `[ -r <path> ]`, the same probe the checklist bullet below names, and named here for the same reason: a promise whose probe is unstated is a promise nothing performs. The key is what carries the class — a value may name any file, so `workspace` is recoverable from the key alone and from nothing else. `understandPath` = the active task's `understand.md`; `planPath` = its `plan.md` (the Business Contract auditor reads the `Business Frame`, `Contract`, and `Decision Register` from these). `testCommand` = `commands.test` from `.ai-flow/project.yml` — the prover runs it to settle what a proposal shows, and reports the proposal unproven rather than guessing when it has none.
   ...
   **Resolve the review profile**, from the same `.ai-flow/project.yml`, keyed on the `area` already resolved above — the identical key the `steering` map uses, which is why it needs no resolution of its own:
   - `review_profile[<area>]`, else `review_profile.default` **where the operator wrote one**, else **nothing**. An area that resolves to nothing is reviewed with the engine's own **generic**, stack-agnostic lists, and **no project default is ever inferred** — an omission makes the review shallower, never wrong. `default:` is a reserved key inside `review_profile:` and never an area name, and **`engine-generic` is a reserved value** inside it and never a profile name — it is the operator saying that this area is judged by the engine's generic lists on purpose. Both reservations are stated together because a reader who meets one and not the other reads the other as some project's own name.
   - **The reserved value is recognised before the `review[<profile>]` lookup**, so it is never looked up in `review:` **as a profile to resolve** — that one reading of the key is what it is exempt from, not the key itself; a set declared under the name is still seen, and the next bullet says what becomes of it — and can therefore never be reported as a declaration that did not resolve. Read in the other order, the one declaration whose whole purpose is to silence the line below would be the declaration that produces it.
   - **A profile declared in `review:` under the reserved name is not silently shadowed.** The reserved value wins — it must, or the switch's meaning would depend on what a project happens to have declared elsewhere. What the run and the report owe in that case is the exception on the table's last row below.
   - The resolved profile is a map of axis → checklist path in `review[<profile>]`. **Test each path before the Workflow call** — `[ -r <path> ]`, named here the way step 3 names `git rev-parse --verify --quiet` and step 8 names its `diff -q`, because a promise whose probe is unstated is a promise nothing performs. A path that reads is passed as the matching `*Checklist` argument; a path that does not is **dropped from the args**, so that axis reviews with the engine generic. An axis the profile does not declare is passed nothing, and that is the ordinary case, not an error: **nothing is reported as a problem**. What each resolution obliges the run and the report to say is the table below.
   - **The test is on what **resolved**, never on whether the keys are present.** The shipped template
     hands every adopter both keys commented out, so a rule keyed on key presence would stay silent on
     precisely the majority case — the project that declared nothing, was read against the generic lists,
     and was never told. An omission and a decision read identically from outside, and only one of them
     was made, which is what the table's last row turns on.

   **What each resolution says, and where.** Five outcomes against two channels — the run's own output at
   this step, before any auditor runs, and the report at step 10. **This table is the only statement of
   them**: the bullets above say what *resolves*, and step 10 and the report template cite this table and
   restate no row of it. The count in that first sentence is not a claim to be trusted — it is checked
   against the rows below, so a row added or dropped without a reader noticing is a red row rather than a
   sentence that has quietly become false.

   | Outcome | What the run says here | What step 10 records |
   |---|---|---|
   | A profile resolved | which profile resolved, and the checklist each axis received | the profile, and the checklist each axis received — naming any axis that received none, which is the ordinary case and still has to be recorded |
   | The profile is absent from `review:` | names the declaration that did not resolve; the whole area falls to the engine generic; the run continues | the profile path that did not resolve |
   | A checklist path does not read | names the path; **that axis alone** falls to the generic, every other axis keeps what it resolved | that the axis received none, and the path that did not resolve |
   | The area resolved to no profile | one line: the change was read against **the engine's generic lists only**, with the remedy inline — declare `review:` and `review_profile:` for this area to sharpen it, or `review_profile: <area>: engine-generic` to accept the generic on purpose | the same line, and what to declare to sharpen it |
   | The area resolved to `engine-generic` | nothing about profiles — an explicit written choice is the only legitimate zero | nothing about profiles |

   **The last row has one exception**, and it is the only case that can arise inside it: a profile declared
   in `review:` under the reserved name. The reserved value still wins, and the shadowing is disclosed in
   both channels — the run says that declaration was shadowed by the reserved name, and step 10 records it.
   A project that has just lost its own checklists learns it in one line rather than from a review that ran
   shallower than it asked for. It rides on that row rather than being a sixth outcome because the
   exemption is what creates it.

   **Every row above reaches the run's own output, not only the rows that report a failure**: the operator
   who declared a profile is the one this feature was built for, and a run that tells them only when they
   got it wrong leaves them reading the report afterwards to learn what judged their change. Same shape
...
8. **Compare the working copy against the copy taken in step 6.** Recompute the path — `SNAP="${TMPDIR:-/tmp}/ai-flow-verify-T-XXX"` — so the value this step uses is the one this step derived.
