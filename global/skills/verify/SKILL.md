...
7. **Invoke the verify-review workflow** (deterministic 5-auditor + adversarial refutation). **Copy the script into the scratchpad first** — `cp ~/.claude/workflows/verify-review.js <scratchpad>/verify-review.js` — and pass that path. The Workflow tool accepts only a path it returned or one you can already read, which `~/.claude/` is not: passing the installed path directly is refused, and the refusal names no remedy. Copy per run, never once and reused, or an edit to the installed workflow silently stops reaching the review. Call the **Workflow** tool with:
   ...
   Resolve `area` from the affected unit using `area_kind` in `.ai-flow/project.yml`. Resolve **`contextPaths`** — a list of `{ key, path }` entries, in resolution order — by the rule in `~/.claude/ai-flow/protocols/context.md` > `## Reading`, **which owns it and is not restated here**: the reserved `workspace` entry, every `steering:` key the changed files make affected, and the fixed `product.md` and `decisions-global.md`. Each file once. What is delivered is the map's **value**, whatever path it names — resolving to a directory instead would silently drop every entry naming something else. **A path that does not read is named in the run's own output and dropped from the list**, never dropped in silence: a path nobody looked up cannot be reported missing. The key is what carries the class — a value may name any file, so `workspace` is recoverable from the key alone and from nothing else. `understandPath` = the active task's `understand.md`; `planPath` = its `plan.md` (the Business Contract auditor reads the `Business Frame`, `Contract`, and `Decision Register` from these). `testCommand` = `commands.test` from `.ai-flow/project.yml` — the prover runs it to settle what a proposal shows, and reports the proposal unproven rather than guessing when it has none.
