...
- **The material leg.** A phase that consumes what an earlier one produced checks it is there: `plan`
  ...
  refusal, and the absence is deliberate**: an author who cannot write a falsifier has a criterion that
  gate already refuses, and an escape hatch readmits what the field exists to stop.
  `verify` needs `plan.md` and the Criteria Coverage table inside it, which is
...
- **Session-close changelog entries** (`> YYYY-MM-DD — ...`) go to `archive/CHANGELOG.md` (newest first) — that file is the **only** home, written **once**, at close time. BACKLOG.md keeps **one line per entry** — a pointer to its `archive/CHANGELOG.md` record, never a condensed retelling — of the **3 most recent** as session continuity; when adding a new one, the oldest of the 3 is **deleted from BACKLOG.md, NOT moved anywhere** — it is already in `archive/CHANGELOG.md`. This copy is one of the **six index surfaces** a shared 25-word ceiling holds every session-written line to (the Icebox's own bullet, BACKLOG's task/epic rows, the roster's rows, `archive/EPICS.md`'s rows, an epic's own Execution Order lines, and this changelog copy) — the number is stated once, at `### Icebox (discoveries)` below (**"The line has a budget: 25 words..."**), and never restated independently here.
