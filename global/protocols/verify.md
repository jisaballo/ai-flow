...
3. **Contract check**: diff the current conformance specs against `artifacts/T-XXX/conformance-baseline/manifest.md`. Every frozen row must still exist with the same `it()` description, assert direction and **falsifier**, and every recorded absence must still carry the same cause; any divergence must have a matching `## Implementation Decisions` entry in understand.md. Divergence without entry -> ❌. The falsifier is compared because a row whose falsifier may be rewritten in flight is frozen in name only — it is the field that says what would make the criterion false, and the author who rewrites it is the same actor the field exists to supply a second source against.
...
6. **Skip-marker grep**: the task diff's **added lines** must switch no test off — the rule lives in CLAUDE.md > *Never*: skip or disable tests, and until now nothing anywhere read it. Two legs and one base, exactly as step 5 has them. Unlike step 5 the restriction is **inside the command rather than beside it**: `TESTS` is the subset of the diff's own files that the diff brake's `TEST_RE` (`~/.claude/hooks/diff-size-guard.py`) accepts — one statement of what a test file is, two readers of it — and it is the command's **pathspec**, so the leg cannot read a file it is not entitled to. A restriction stated in prose beside a command that does not carry it is not a second reader of anything; it is an adjective. **The marker set is declared here and only here** — the `/verify` skill names the leg and routes to this line rather than restating it, because a second copy of ten patterns is a set that grows in one reader and not the other:
   ...
   ```sh
   TESTS="$(git diff --name-only <merge-base> | <the names TEST_RE accepts>)"   # empty => the leg has no subject
   git diff <merge-base> -U0 -- $TESTS \
     | awk '/^\+\+\+ /{f=substr($0,7);next} /^@@/{n=$3;sub(/^\+/,"",n);sub(/,.*/,"",n);next} /^\+/{print f":"n": "$0;n++}' \
     | grep -E '(\.skip\(|\.only\(|\bxit\(|\bxdescribe\(|\bfit\(|\bfdescribe\(|@pytest\.mark\.skip|\bt\.Skip\(|@Disable[d]|@Ignor[e])'
   ```

   and the same over the files `git ls-files --others --exclude-standard` names, put through `TEST_RE` the same way. **The `awk` stage is what makes the promise payable**: `grep -n` over a piped diff numbers the *diff*, so it can report neither the file nor the line this rule asks for, while the hunk headers carry both — `-U0` is what lets the count be kept by walking added lines alone. The *focus* family is in the set because `it.only(` disables every **other** test in the file — the same failure in the opposite shape, and the one the rule's own wording does not suggest. **Four of the ten take a word boundary, and it is load-bearing rather than decorative**: unanchored, `xit\(` matches `sys.exit(` and `process.exit(`, and `fit\(` matches `benefit(` and `profit(` — this engine's own suite offered ten such lines and not one true positive, and a blocking gate whose first firing is false is the gate that gets routed around. Hits -> ❌ with the **file and the line** (fix before archive; a test switched off to reach green is the defect this engine is least able to see afterwards, since the suite it broke reports nothing). Every pattern is written so that **its own text is not an instance of it** — the escaped forms break themselves, the two bare ones take a single-character class, and a word boundary changes neither — which is what keeps this protocol, the skill and the conformance suite committable and greppable with the leg live; exempting their paths instead would create three files in which a real violation passed unseen. A path the diff cannot show as text, and the split between tracked and untracked work, are step 5's existing problem and are inherited here rather than re-solved.

7. **Re-run all Verify commands** from each plan.md step (catches cross-step regressions — a later step may have broken an earlier one)
...
Runs 5 auditors in parallel over the task diff (see **The Task Diff** above): the **Business Contract
Auditor**, the **Test Coverage Auditor**, **Security & Error Handling**, **Architecture Boundaries**, and
...
### Simplicity & Structure
- [finding + the simpler design it proposed, or "No structural cost found"]
