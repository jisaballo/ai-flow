...
6. **Skip-marker grep**: the task diff's **added lines** must switch no test off — the rule lives in CLAUDE.md > *Never*: skip or disable tests, and until now nothing anywhere read it. Two legs and one base, exactly as step 5 has them, restricted to the files the diff brake's own `TEST_RE` (`~/.claude/hooks/diff-size-guard.py`) recognises as tests: one statement of what a test file is, two readers of it. **The marker set is declared here and only here** — the `/verify` skill names the leg and routes to this line rather than restating it, because a second copy of ten patterns is a set that grows in one reader and not the other: `git diff <merge-base> | grep -nE '^\+.*(\.skip\(|\.only\(|xit\(|xdescribe\(|fit\(|fdescribe\(|@pytest\.mark\.skip|t\.Skip\(|@Disable[d]|@Ignor[e])'`, and the same over the files `git ls-files --others --exclude-standard` names. The *focus* family is in the set because `it.only(` disables every **other** test in the file — the same failure in the opposite shape, and the one the rule's own wording does not suggest. Hits -> ❌ with the **file and the line** (fix before archive; a test switched off to reach green is the defect this engine is least able to see afterwards, since the suite it broke reports nothing). Every pattern is written so that **its own text is not an instance of it** — the escaped forms break themselves, and the two bare ones take a single-character class — which is what keeps this protocol, the skill and the conformance suite committable and greppable with the leg live; exempting their paths instead would create three files in which a real violation passed unseen. A path the diff cannot show as text, and the split between tracked and untracked work, are step 5's existing problem and are inherited here rather than re-solved.
