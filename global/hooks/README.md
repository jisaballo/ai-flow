...
- **`-f` is true for a file whose contents cannot be read** — stat only needs the parent directory. So
  is `-d` for a directory that cannot be entered, which matters because an unreadable directory makes
  the files inside it test as *absent*, i.e. as the designed silence. `-r` is the test that separates
  them, and it is needed beside `-f`, not instead of it.
...
One thing this does not close: a read that fails *after* the readability test passes. Where the status is
free to capture, capture it and the window shuts; where it is not, the window is accepted and named here
