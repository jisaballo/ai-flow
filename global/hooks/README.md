...
| `drift-check.sh` | Stop | Engine drift guard. Quiet while the clone has uncommitted engine changes; blocks session close when an installed engine file differs from the clone's committed HEAD, printing the exact fix. No-op on remote installs (no clone known). |
