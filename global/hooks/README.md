...
| `context-cost-note.py` | Stop | Session cost note, and the only hook here that **never refuses**. Measured over 285 transcripts: cache-read is ~70% of the price-weighted bill and cache-read is context size times turns, so what a turn does barely moves its cost -- only how much context it drags; sessions of 150 turns or more are 95.7% of this engine's own repository's bill. Simulated on each session's real per-turn curve with re-caching priced in, restarting every 150 turns saves **24.3%** here and **32.9%** on the project the engine is used to build. Those are the corrected figures: an earlier estimate of 51.3% modelled a restart from zero context, and turn 1 already costs 43-47k for the system prompt, the project's instructions and the tool definitions -- that floor is why the honest number is a quarter to a third. So at 150 main-loop turns it says once that cutting at the **next phase boundary** now pays, and again at 300, firmer, and never a third time. It names what a cut destroys rather than saying "save your state": the sheet's `## Ruled out` lines, a `hypothesis:` tested and killed and an `alternative:` rejected with its reason, neither of which is a decision *taken* and so neither of which the sheet used to hold. Two things about how it speaks are load-bearing. It **exits 0 on every path**, because blocking mid-Execute costs more than the tokens it saves -- and that decides the channel, since a Stop hook that exits 0 has its **stderr discarded** by the harness, so its message travels as a top-level `systemMessage` on **stdout** and would reach nobody on stderr. And it keeps **no sentinel file**: the harness records a delivered `systemMessage` back into the session's own transcript, so the hook reads its own prior note out of the file it already opens -- no path to choose, no session-versus-checkout scope to decide, nothing left behind. Turns are counted from `transcript_path` (else the newest record for this working copy), as assistant records carrying `usage`, skipping any marked `isSidechain`: a subagent's turns are not in this session's context and are 1.2% of the bill. Every failure to read or parse falls to **silence**, never to a note. No-op in any project without an `.ai-flow/`. |
